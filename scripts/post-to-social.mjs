...
// Mastodon post (single 1500-char chunk, truncate + read more)
async function postToMastodon(baseUrl, token, text, socialPreview = true, link = '') {
  const LIMIT = 1500;
  let statusText;

  if (socialPreview && text.length + link.length + 11 > LIMIT) { // 11 = length of "… read more: "
    const allowed = LIMIT - link.length - 11;
    statusText = text.slice(0, allowed).trim() + `… read more: ${link}`;
  } else {
    statusText = `${text}\n${link}`;
  }
  ...
  console.log('Mastodon posted:', json.id);
}
...
async function postToBluesky(username, appPass, text, socialPreview = true, postLink = '') {
  ...
  if (socialPreview) {
    // footer always added
    const footer = `… read more: ${postLink}`;
